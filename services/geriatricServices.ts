...
export const fetchGeriatrics = async ({
  ...
  if (filters.stayType.permanentStay)
    params.append("hasPermanentStay", "true");
  ...
  if (filters.dependency.independent)
    params.append("hasIndependentCare", "true");
  if (filters.dependency.semiDependent)
    params.append("hasSemiDependent", "true");
  if (filters.dependency.dependent) params.append("hasDependent", "true");
  if (filters.dependency.highComplexity)
    params.append("hasHighComplexity", "true");

  // Medical Filters
  if (filters.medical.nursing24)
    params.append("has24hMedical", "true");
  if (filters.medical.presentialDoctor)
    params.append("hasPresentialDoctor", "true");
  if (filters.medical.neurological)
    params.append("hasAttentionForNeurologicalDiseases", "true");
  if (filters.medical.medication)
    params.append("hasMedicationSupply", "true");
  // Price Range
  ...
  const response = await axios.get(
    `/api/geriatrics/search?${params.toString()}`
  );
